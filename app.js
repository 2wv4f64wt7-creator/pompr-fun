...
if (PFState.character) {
    const characterObj = PFState.csvData.characters.find(
        c => c.name === PFState.character
    );

    const characterText = characterObj
        ? `${characterObj.name}, ${characterObj.desc}`
        : PFState.character;

    parts.push(characterText);
}
...
------------------------------*/
if (PFState.action) {
    const actionObj = PFState.csvData.actions.find(
        a => a.name === PFState.action
    );

    const actionText = actionObj?.desc || PFState.action;
